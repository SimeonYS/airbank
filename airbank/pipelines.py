...
class AirbankPipeline:

    # Database setup
    conn = sqlite3.connect('airbank.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `airbank`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM airbank WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO airbank (date, title, link, content)"
